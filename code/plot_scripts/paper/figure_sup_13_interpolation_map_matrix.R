...
lapply(
  ...
  p_absolute_speed <- mobility_proxy %>%
    ...
    #   aes(x, y), alpha = 0.8, color = "grey", shape = 4
    # ) +
    scale_fill_viridis_c(option = "inferno", direction = -1, limits = c(0, 500)) +
    theme_bw() +
    coord_sf(
      xlim = xlimit, ylim = ylimit,
      crs = epsg102013
  ...
  p_alternative_speed <- alternative_mobility_proxy %>%
    dplyr::filter(
      kernel_setting_id == kernel_setting,
      z %in% seq(-7500, 1500, 500)
    ) %>%
  ...
  # merge plots
  p <- cowplot::plot_grid(p_Meas, p_C1, p_C2, p_Change, p_absolute_speed, p_alternative_speed, ncol = 6)
