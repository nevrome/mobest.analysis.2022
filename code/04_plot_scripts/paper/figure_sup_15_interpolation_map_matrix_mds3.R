...
p_C3 <- interpol_grid %>%
  ...
  guides(
    fill = guide_colorbar(title = "Prediction C3  ", barwidth = 25)
  ) +
  theme(
    legend.position = "bottom",
    legend.box = "horizontal",
...
C3_legend <- cowplot::get_legend(p_C3)
p_C3 <- p_C3 + theme(legend.position = "none")

plots <- cowplot::plot_grid(p_C1, p_C2, p_C3, nrow = 3)
legends <- cowplot::plot_grid(C1_legend, C2_legend, C3_legend, nrow = 3)
