...
spatiotemporal_distances <- janno_initial_selection %$%
  data.frame(
...
group_representatives <- identical_groups %>%
  dplyr::left_join(
    janno_initial_selection, by = c("id" = "Poseidon_ID")
  ) %>%
  dplyr::group_by(group) %>%
  dplyr::filter(
    Nr_SNPs == max(Nr_SNPs)
...
# remove identicals
`%nin%` <- Negate(`%in%`)
janno_without_identicals <- janno_initial_selection %>%
...
tibble::tibble(
  ...
  readr::write_delim(
    file = "code/01_data_preparation/ind_list_remove_related_selection.txt",
    delim = " ",
    col_names = FALSE
  )
